...
- `storage/` — BadgerDB key-value store for persistent config (ADB path, etc).
- `types/` — Shared `ExecResult` struct.
- `util/` — Shell execution, Android version mapping, string helpers.
...
- **Views:** `RootContainer.tsx` switches between 5 views: actions (RightContainer), app list (ApplicationList), memory monitor (MemoryMonitor), device info (DeviceInfoCard), FAQ, settings.
- **Actions:** `frontend/src/data/quickActions.ts` defines 50+ quick actions shown in the main panel.
